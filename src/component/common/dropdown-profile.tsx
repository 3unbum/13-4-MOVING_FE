...
export default function DropdownProfile({
  ...
  return (
    <div
      ...
      className={clsx(
        "border-line-200 inline-flex flex-col overflow-hidden border bg-gray-50 shadow-[2px_2px_8px_rgba(224,224,224,0.2)]",
      ...
      {header ? (
        <div
          className={clsx(
            ...
            isSm ? "px-3 py-2" : "py-3.5 pr-3 pl-6"
          )}
        >
          <div className={clsx("text-black-400 font-bold", isSm ? "text-16" : "text-18")}>
            {header}
          </div>
        </div>
      ) : null}
      ...
      {defaultOptions.map((option) => (
        <li key={option.value} role="none" className="w-full">
          <button
            ...
            className={clsx(
              "text-black-400 hover:bg-background-300 flex w-full items-center gap-0.5 bg-gray-50 text-left font-medium",
              isSm ? "text-14 px-3 py-2" : "text-16 py-3.5 pr-3 pl-6",
      ...
      {mutedOptions.map((option) => (
        <li key={option.value} role="none" className="w-full">
          <button
            ...
            onClick={() => onSelect(option.value)}
            className={clsx(
              "border-line-100 hover:bg-background-200 flex w-full items-center justify-center gap-0.5 border-t bg-gray-50 text-gray-500",
              isSm
                ? "text-12 px-3 pt-3 pb-3.5 font-normal"
                : "text-14 px-3 pt-3.5 pb-3.5 font-medium"
            )}
          >
            {option.label}
          </button>
